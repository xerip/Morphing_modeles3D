...
void MainWindow::on_pushButton_superMesh_clicked()
{
    ...
    // Sphériser Src
    if (b_srcSphere)    delete mySphereSrc;
    mySphereSrc = new Sphere(meshSrcSphere, origin);
    spheriser(WINDOW_1, rayonGeneral);

    // Sphériser Tgt
    if (b_tgtSphere)    delete mySphereTgt;
    mySphereTgt = new Sphere(meshTgtSphere, origin);
