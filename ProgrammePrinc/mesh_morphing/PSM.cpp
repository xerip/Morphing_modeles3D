...
PSM::PSM(MyMesh &_meshSrc, MyMesh &_meshTgt,
...
MyMesh PSM::get_superM()  {   return superM;  }

/*--------------------------------------------------------------------------
 * Fonction de coordonnees barycentriques.
 * retourne les coordonnes barycentriques du point P dans le triangle ABC.
 * -------------------------------------------------------------------------*/
QVector3D PSM::coords_barycentre(QVector3D A, QVector3D B, QVector3D C, QVector3D P)
{
...
MyMesh::Point PSM::my_coords_barycentrique(MyMesh *_mesh, unsigned numFace, MyMesh::Point P)
{
 ...
 * -------------------------------------------------------------------------*/
VertexHandle PSM::get_plus_proche_vh_of_triangle(MyMesh *_mesh, VertexHandle vh1, VertexHandle vh2, VertexHandle vh3, MyMesh::Point p, vector<bool> vertex_used)
{
...
VertexHandle PSM::get_plus_proche_vh_of_triangle_bis(MyMesh *_mesh, int numFace, vector<bool> vertex_used)
{
 ...
 * -------------------------------------------------------------------------*/
MyMesh::Point PSM::my_linear_interpolation(MyMesh::Point p0, MyMesh::Point p1, float t)
{
    if (t<0.f || t>1.f) {
 ...
 * -------------------------------------------------------------------------*/
bool PSM::my_intersect(MyMesh *_mesh, int face, MyMesh::Point O, MyMesh::Point P,
                             MyMesh::Point &resP)
{
...
void PSM::add_pos_vh_split_Tgt_to_Src()
{
    qDebug() << "\t<" << __FUNCTION__ << ">";
...
void PSM::handle_intersection(unsigned numFaceSM, MyMesh::Point pSrc,
                                    MyMesh::Point pIntersect, vector<bool> &vertex_used)
{
    ...
    //on recupere le vh du plus proche du point intersecte avec le PSM qui n'a pas deja ete pris (sauf si tous sont pris)
...
void PSM::build()
{
    ...
    //add_pos_src du vertex_split qui intersecte le meshSrc depuis le PSM
 ...
 * -------------------------------------------------------------------------*/
unsigned PSM::create_new_point_middle_edge(unsigned posF, unsigned posV, unsigned posV2)
{
...
void PSM::my_split(int numFace)
{
    ...
    while (!flag_face_complete)
    {
        ...
        for (unsigned i=0; i<myF.size(); i++)
        {
            ...
            if (myV.type == VERTEX_SPLIT)   continue;
            ...
            if (myV2.type == VERTEX_SPLIT)  continue;
