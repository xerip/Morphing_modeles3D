    ...
    QMainWindow(parent), ui(new Ui::MainWindow)
{
    ...
    // myMetaM = new Sphere(mesh, origin);
...
/*--------------------------------------------------------------------------------
 *  Modifie la taille globale de @_mesh2 pour correspondre à la taille de @_mesh1
 * ------------------------------------------------------------------------------*/
void MainWindow::modify_mesh_size(MyMesh *_mesh1, MyMesh *_mesh2)
{
    MyMesh::Point myOrigine1;
    deplacement_origine(_mesh1);
    float myRayon1 = rayon_Englobant(_mesh1, myOrigine1);

    MyMesh::Point myOrigine2;
    deplacement_origine(_mesh2);
    float myRayon2 = rayon_Englobant(_mesh2, myOrigine2);

    float myRayon = myRayon1 - myRayon2;

    for (MyMesh::VertexIter v_it = _mesh2->vertices_begin(); v_it != _mesh2->vertices_end(); v_it++)
    {
        QVector3D vP, vO, myV3D;
        //precision = true;
        float t = myRayon;
        float pas = t;
        VertexHandle vh = *v_it;
        MyMesh::Point P = _mesh2->point(vh);

        MyMesh::Point myVec = P - origin;
        vP = QVector3D(P[0], P[1], P[2]);
        vO = QVector3D(origin[0], origin[1], origin[2]);
        myV3D = RayCasting::get_point_of_line(vP, vO, pas);
        myVec = MyMesh::Point (myV3D[0], myV3D[1], myV3D[2]);
        _mesh2->set_point(vh, myVec);
    }
}
...
void MainWindow::display_vertex_control(MyMesh *_mesh, std::vector<int> list_num_vertex)
{
    ...
    {
        if (static_cast<unsigned>(list_num_vertex[i])<(_mesh->n_vertices()))
        {
...
//    // Verif
//    if (!_Sphere->verif_rayon(origin, rayon)) {
//        qDebug() << "<\t" << __FUNCTION__ << ">";
//        qDebug() << "\t\trayon trop court -> " << rayon;
//        qDebug() << "\t</" << __FUNCTION__ << ">";
//        return;
//    }
    _Sphere->spherization(rayon);
    if (windowNum == WINDOW_1) {
        distsPointsBase1 = _Sphere->_distPointsBase();
    }
    else if (windowNum >= WINDOW_2){
        distsPointsBase2 = _Sphere->_distPointsBase();
    }
...
void MainWindow::make_psm()
{
    qDebug() << "<" << __FUNCTION__ << ">";

    if ( !(b_srcSphere &&  b_tgtSphere) ) return;

    mySM = new PSM(meshSrc, meshTgt,
                         meshSrcSphere, meshTgtSphere,
                         superM, origin);
    //    MyMesh superM = mySM->get_superM();
    mySM->build();

    mySM->myData.display_vertices();

    resetAllColorsAndThickness(&superM);
...
void MainWindow::on_pushButton_superMesh_clicked()
{
    ...
    meshSrcSphere = meshSrc;
    MyMesh::Point myOriginSrc;
    deplacement_origine(&meshSrcSphere);
    float rayonSrc = rayon_Englobant(&meshSrcSphere, myOriginSrc);
    qDebug() << "\t rayonSrc = " << rayonSrc;

    meshTgtSphere = meshTgt;
    MyMesh::Point myOriginTgt;
    deplacement_origine(&meshTgtSphere);
    float rayonTgt = rayon_Englobant(&meshTgtSphere, myOriginTgt);
    ...
    // Sphériser Src
    if (b_srcSphere)    delete myMetaSrc;
    myMetaSrc = new Sphere(meshSrcSphere, origin);
    spheriser(WINDOW_1, rayonGeneral);

    // Sphériser Tgt
    if (b_tgtSphere)    delete myMetaTgt;
    myMetaTgt = new Sphere(meshTgtSphere, origin);
    ...
    ui->pushButton_fusion->setEnabled(true);
...
void MainWindow::on_pushButton_spherizationSrc_clicked()
{
    qDebug() << "<" << __FUNCTION__ << ">";
    if ( b_srcSphere ) {
        displayMesh(&meshSrcSphere, WINDOW_1);
        return;
    }

    meshSrcSphere = meshSrc;
    MyMesh::Point myO;
    float myRayon = rayon_Englobant(&meshSrcSphere, myO);
    myMetaSrc = new Sphere(meshSrcSphere, origin);
...
void MainWindow::on_pushButton_spherizationTgt_clicked()
{
    qDebug() << "<" << __FUNCTION__ << ">";
    if ( b_tgtSphere) {
        displayMesh(&meshTgtSphere, WINDOW_2);
        return;
    }

    meshTgtSphere = meshTgt;
    MyMesh::Point myO;
    float myRayon = rayon_Englobant(&meshTgtSphere, myO);
    myMetaTgt = new Sphere(meshTgtSphere, origin);
...
void MainWindow::on_doubleSpinBox_pourcentage_valueChanged(double val)
{
    qDebug() << "<" << __FUNCTION__ << ">";
    pourcentage = static_cast<float>(val);
    qDebug() << "\tpourcentage t =" << pourcentage;
    qDebug() << "</" << __FUNCTION__ << ">";
...
void MainWindow::on_pushButton_chargement_2_clicked()
{
    ...
    modify_mesh_size(&meshSrc, &meshTgt);
