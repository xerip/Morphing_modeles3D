...
void MainWindow::affiche_vertex_control(MyMesh* _mesh, std::vector<int> list_num_vertex)
{
...
void MainWindow::on_add_Vertex_clicked()
{
    ...
    if (!occ) list_vertex_control_src.push_back(val); ui->label->setText("valeur ajouté : " + QString::number(val));
    std::sort(list_vertex_control_src.begin(),list_vertex_control_src.end());
    for (unsigned int i=0; i<list_vertex_control_src.size(); i++) std::cout<<list_vertex_control_src[i]<< " ";
    std::cout << std::endl;
}


void MainWindow::on_add_vertex_2_clicked()
{
    ...
    if (!occ) {list_vertex_control_tgt.push_back(val_2); ui->label->setText("valeur ajouté : " + QString::number(val_2));}
    std::sort(list_vertex_control_tgt.begin(),list_vertex_control_tgt.end());
    for (unsigned int i=0; i<list_vertex_control_tgt.size(); i++) std::cout<<list_vertex_control_tgt[i]<< " ";
    std::cout << std::endl;
}
...
{
    resetAllColorsAndThickness(&mesh_src);
    VertexHandle vh = mesh_src.vertex_handle(num_vertex);
    mesh_src.set_color(vh, MyMesh::Color(255, 255, 0));
    mesh_src.data(vh).thickness = 15;
    displayMesh(&mesh_src);
}

void MainWindow::on_spinBox_2_valueChanged(int num_vertex)
{
    resetAllColorsAndThickness(&mesh_tgt);
    VertexHandle vh = mesh_tgt.vertex_handle(num_vertex);
    mesh_tgt.set_color(vh, MyMesh::Color(255, 255, 0));
    mesh_tgt.data(vh).thickness = 15;
    displayMesh_2(&mesh_tgt);
}
...
void MainWindow::on_pushButton_chargement_clicked()
{
    ...
    OpenMesh::IO::read_mesh(mesh_src, fileName.toUtf8().constData());

    mesh_src.update_normals();

    // initialisation des couleurs et épaisseurs (sommets et arêtes) du mesh
    resetAllColorsAndThickness(&mesh_src);

    /*std::vector<int> tab_vertex;
    tab_vertex.push_back(1);
    tab_vertex.push_back(3);
    tab_vertex.push_back(5);
    tab_vertex.push_back(17);
    tab_vertex.push_back(21);
    tab_vertex.push_back(31);
    tab_vertex.push_back(51);
    tab_vertex.push_back(81);
    tab_vertex.push_back(200);
    tab_vertex.push_back(425);
    tab_vertex.push_back(523);
    tab_vertex.push_back(412);
    tab_vertex.push_back(126);
    tab_vertex.push_back(6);
    tab_vertex.push_back(12);*/
    //affiche_vertex_control(&mesh_src, tab_vertex);
    // on affiche le maillage
    displayMesh(&mesh_src);
    displayMesh(&mesh_src);;
    //displayMesh_2(&mesh);
    ui->spinBox->setEnabled(true);
    ui->spinBox->setRange(0,mesh_src.n_vertices()-1);
...
void MainWindow::on_pushButton_chargement_2_clicked()
{
    ...
    // chargement du fichier .obj dans la variable globale "mesh"
    OpenMesh::IO::read_mesh(mesh_tgt, fileName.toUtf8().constData());

    mesh_tgt.update_normals();
    ...
    tab_vertex.push_back(6);
    tab_vertex.push_back(12);
    affiche_vertex_control(&mesh_tgt, tab_vertex);*/

    displayMesh_2(&mesh_tgt);
    displayMesh_2(&mesh_tgt);

    ui->spinBox_2->setRange(0,mesh_src.n_vertices()-1);
...
void MainWindow::displayMesh_2(MyMesh* _mesh, bool isTemperatureMap, float mapRange)
{
    GLuint* triIndiceArray = new GLuint[_mesh->n_faces() * 3];
    GLfloat* triCols = new GLfloat[_mesh->n_faces() * 3 * 3];
    GLfloat* triVerts = new GLfloat[_mesh->n_faces() * 3 * 3];
