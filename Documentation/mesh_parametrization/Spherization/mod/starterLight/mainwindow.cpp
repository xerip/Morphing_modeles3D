...
vector<float> MainWindow::K_Curv(MyMesh* _mesh)
{
    vector <float> kList;
    // courbure gaussienne
...
int chercher_sommet_kmax(vector<float> kList)
{
    float max = 0.f;
    int iMax = -1;
...
void MainWindow::algo(MyMesh *_mesh)
{
    int cpt=0;
    vector<float> kList = K_Curv(_mesh);
    qDebug() << kList;
...
//    }
    ...
    // Dist euclidienne
    /*
     * AB = sqrt ( pow(xB-xA,2) + pow(ybB-yA,2) + pow(zB-zA,2) );
     * */

    while (1)
    {
        int v = chercher_sommet_kmax(kList);
